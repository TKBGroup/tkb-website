...
// --- Helper Components ---
// 1. Magnetic Button for hover effect
...
export default function Hero() {
    ...
    // State for the magnetic text effect
    ...
    const handleMouseMove = (e: React.MouseEvent<HTMLButtonElement>, setPosition: Function) => {
        const { clientX, clientY } = e;
        const { height, width, left, top } = e.currentTarget.getBoundingClientRect();
        const x = (clientX - (left + width / 2)) * 0.15; // Multiplier controls intensity
    ...
    return (
        <section className="relative h-auto py-36 px-20 w-full overflow-hidden text-white"> 
            <AnimatePresence>
                <motion.div
                    key={index}
            ...
            <div className="relative z-10 flex h-full flex-col justify-center container mx-auto px-6">
                <div className="max-w-3xl">
                    ...
                    <h1 className="mt-4 text-5xl md:text-7xl font-semibold leading-tight">
                       Elevate Your Space with TKB Group </h1>
                    <p className="mt-6 text-lg text-gray-300">
                        Decades of delivering custom cabinetry in Ontario, British Columbia, and Alberta.
                    </p>
                </div>
  ...
  hover:bg-yellow-500/80"
                    >
                        <span
                            style={{ transform: `translate3d(${contactPos.x}px, ${contactPos.y}px, 0)` }}
  ...
  hover:bg-gray-900/80 hover:border-gray-900/0"
                    >
                        <span
                            style={{ transform: `translate3d(${estimatePos.x}px, ${estimatePos.y}px, 0)` }}
                            className="relative z-10 inline-block transition-transform duration-150 ease-out"
                        >
            ...
            <div className="absolute bottom-6 left-6 z-10 flex items-center gap-2 text-sm uppercase cursor-pointer" onClick={handlePrev}>
            ...
            <div className="absolute bottom-6 right-6 z-10 flex items-center gap-2 text-sm uppercase cursor-pointer" onClick={handleNext}>
                <span>Next</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>
            </div>
 

            <div className="hidden lg:block absolute bottom-1/2 right-6 translate-y-44 z-10 mr-12">
                <MagneticButton onClick={handleScrollDown} className="group flex h-30 w-30 items-center justify-center rounded-full border border-white transition-colors duration-300 hover:bg-white hover:text-black">
                    <svg className="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 13.5L12 21m0 0l-7.5-7.5M12 21V3" /></svg>
                </MagneticButton>
            </div>
        </section>
    );
